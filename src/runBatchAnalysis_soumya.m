...
function [memoryUsage, totalMem, elapsedTime] = runBatchAnalysis(runA, runB, runC, runD, runE, runF, gDate, gRun, workingOnServer, diaryOn, profilerTest)
...
make_dbSoumya_single %in localCopies
...
if loadSyntheticData %Synthetic Data
    ...
    for runi = sdcpStart:1:sdcpEnd
        ...
        if runA
            ...
            %save([saveFolder db(1).mouse_name '_' db(1).date '_methodA.mat' ], 'mAInput', 'mAOutput')
        end
        ...
        if runB
            ...
            %save([saveFolder db(1).mouse_name '_' db(1).date '_methodB.mat' ], 'mBInput', 'mBOutput')
        end
        ...
        if runC
            ...
            %save([saveFolder db(1).mouse_name '_' db(1).date '_methodC.mat' ], 'mCInput', 'mCOutput')
        end
        ...
        if runD
            ...
            %save([saveFolder db(1).mouse_name '_' db(1).date '_methodD.mat' ], 'mDInput', 'mDOutput')
        end
        ...
        if runE
            ...
            %save([saveFolder db(1).mouse_name '_' db(1).date '_methodE.mat' ], 'mEInput', 'mEOutput')
        end
        ...
        if runF
            ...
            %save([saveFolder db(1).mouse_name '_' db(1).date '_methodF.mat' ], 'mFInput', 'mFOutput')
        end
    end
else %Real Physiology Data
    nDatasets = length(db);
    for runi = 1:nDatasets
        ...
        myData = load([ppFolder db(runi).mouse_name '/' db(runi).date '/' db(runi).mouse_name '_' db(runi).date '_' ...,
                         db(runi).trialPhase '_' db(runi).method '_framebin3'  '_gaussianKernel1'  '.mat' ]);
...
tic
        ...
        if runA
            ...
            %Method A - Mehrab's Reliability Analysis (Bhalla Lab)
            mAInput.cellList = 1:1:size(DATA,1); %using all cells
            mAInput.onFrame = 1; %sdcp(runi).startFrame;
...
if saveData
    ...
    if loadSyntheticData
        ...
        if runC
            save([saveFolder db.mouse_name '_' ...
