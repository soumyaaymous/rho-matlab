...
function emptyOutput = runBatchAnalysisOnRealData_soumya(runA, runB, runC, runD, runE, runF, workingOnServer, diaryOn)

tic
close all

if workingOnServer
   HOME_DIR = '/home1/soumyab/soumya_matlab_codes'; %location of matlab codes on bebinca
    saveDirec = '/home1/bhalla/soumyab/timeCellAnalysis/' ; %location for storing synthetic data sets
    ppFolder = '/home1/bhalla/soumyab/CalciumDataAnalysisResults/Preprocessed_files/'; % Location of preprocessed files
else
...
if workingOnServer
   saveFolder = strcat(saveDirec, db.mouse_name, '/', db.date, '/');
else
...
for runi = 1: 1: nDatasets
    ...
    %Save Directory
    mouse_name = dbase(runi).mouse_name;
    date = dbase(runi).date;
    saveFolder = strcat(saveDirec, mouse_name, '/', date, '/');
    
    %Load processed data (processed dfbf for dataset/session)
    myData = load(strcat(saveFolder, mouse_name, '_', date, '.mat'));
    ...
    if runA
        ...
        %save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_methodA.mat' ], 'mAInput', 'mAOutput')
    end
    ...
    if runB
        ...
        %save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_methodB.mat' ], 'mBInput', 'mBOutput')
    end
    ...
    if runC
        ...
        %save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_methodC.mat' ], 'mCInput', 'mCOutput')
    end
    ...
    if runD
        ...
        %save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_methodD.mat' ], 'mDInput', 'mDOutput')
    end
    ...
    if runE
        ...
        %save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_methodE.mat' ], 'mEInput', 'mEOutput')
    end
    ...
    if ops0.saveData
        ...
        if ops0.loadSyntheticData
            if runA
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_synthDataAnalysis_' num2str(gDate) '_gRun' num2str(gRun) '_methodA_batch_' num2str(sdcpStart) '-' num2str(sdcpEnd) '.mat' ], 'mAInput', 'mAOutput_batch', '-v7.3')
            end
            
            if runB
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_synthDataAnalysis_' num2str(gDate) '_gRun' num2str(gRun) '_methodB_batch_' num2str(sdcpStart) '-' num2str(sdcpEnd) '.mat' ], 'mBInput', 'mBOutput_batch', '-v7.3')
            end
            
            if runC
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_synthDataAnalysis_' num2str(gDate) '_gRun' num2str(gRun) '_methodC_batch_' num2str(sdcpStart) '-' num2str(sdcpEnd) '.mat' ], 'mCInput', 'mCOutput_batch', '-v7.3')
            end
            
            if runD
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_synthDataAnalysis_' num2str(gDate) '_gRun' num2str(gRun) '_methodD_batch_' num2str(sdcpStart) '-' num2str(sdcpEnd) '.mat' ], 'mDInput', 'mDOutput_batch', '-v7.3')
            end
            
            if runE
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_synthDataAnalysis_' num2str(gDate) '_gRun' num2str(gRun) '_methodE_batch_' num2str(sdcpStart) '-' num2str(sdcpEnd) '.mat' ], 'mEInput', 'mEOutput_batch', '-v7.3')
            end
            
            if runF
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_synthDataAnalysis_' num2str(gDate) '_gRun' num2str(gRun) '_methodF_batch_' num2str(sdcpStart) '-' num2str(sdcpEnd) '.mat' ], 'mFInput', 'mFOutput_batch', '-v7.3')
            end
        else %Real Physiology Data
            if runA
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_realDataAnalysis_methodA_batch.mat' ], 'mAInput', 'mAOutput_batch', '-v7.3')
            end
            
            if runB
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_realDataAnalysis_methodB_batch.mat' ], 'mBInput', 'mBOutput_batch', '-v7.3')
            end
            
            if runC
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_realDataAnalysis_methodC_batch.mat' ], 'mCInput', 'mCOutput_batch', '-v7.3')
            end
            
            if runD
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_realDataAnalysis_methodD_batch.mat' ], 'mDInput', 'mDOutput_batch', '-v7.3')
            end
            
            if runE
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_realDataAnalysis_methodE_batch.mat' ], 'mEInput', 'mEOutput_batch', '-v7.3')
            end
            
            if runF
                save([saveFolder dbase(runi).mouse_name '_' dbase(runi).date '_realDataAnalysis_methodF_batch.mat' ], 'mFInput', 'mFOutput_batch', '-v7.3')
            end
        end
        disp('... done!')
    end
